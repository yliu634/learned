...
//   Model model;
 ...
 public:
  ...
  KapilLinearExoticHashTableFile(std::vector<std::pair<Key, Payload>> data,
                                 string fileName = "../datasets/oneFile.txt", 
      ...
      // this nested loop construction anyways.
        ...
        for (size_t i = 0; i < BucketSize; i++) {
          ...
          if (current_key == key) {
            ...
            vector<char> buff_;
            pread(storageFile, buff_.data(), buff_.size(), pos * blockSize);
            return 1;
          }
        }
      // if(exit)
